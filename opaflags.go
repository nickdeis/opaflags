...
func FromFilePath(glob_path string, namespace string) OPAFlags {
...
func FromMap(raw_regos map[string]string, namespace string) OPAFlags {
	var regos []*RawRego
...
func FromStructArray(raw_regos []*RawRego, namespace string) OPAFlags {
...
func (f *OPAFlags) EvaluateFlags(input map[string]any) map[string]any {
	timed_context, _ := context.WithTimeout(f.parent_context, time.Millisecond*100)
	results, _ := f.prepped_query.Eval(timed_context, rego.EvalInput(input))
